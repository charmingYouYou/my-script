...
class Build {
  ...
  readProject () {
    return inquirer.prompt({
      ...
      this.projectInfo = config[this.project]
      this.options = {
        cwd: `${this.projectInfo.cwd}${this.project}`,
        encoding: 'utf-8'
      }
      this.buildBranch = this.projectInfo.buildBranch
    })
  }

  async getBranchList () {
    return await exec(`git branch`, this.options).then(({stdout, stderr}) => {
      this.branchList = stdout.trim().replace(/ /g, '').replace('*', '').split('\n')
    }).catch(err => {
      throw err.stderr
    })
  }
